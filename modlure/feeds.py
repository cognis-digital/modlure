"""modlure.feeds — threat-intel enrichment for honeypot events.

MODLURE is a Modbus TCP honeypot: every attacker request becomes a JSON threat
...
by :mod:`modlure.datafeeds`, and re-served with ``offline=True`` so enrichment
...
def enrich_event(event: dict,
    ...
    """Enrich a single MODLURE event in place against the C2/IOC indices.
...
def enrich_events(events: list[dict], *, offline: bool = False,
    ...
    the local cache populated by ``modlure feeds update`` / a snapshot import.
    """
...
# --------------------------------------------------------------------------- #
# CLI glue: `modlure feeds list|update|get <id> [--offline]`
# --------------------------------------------------------------------------- #
def cmd_feeds(args) -> int:
    """Handle the ``modlure feeds`` subcommand (restricted to FEED_IDS)."""
    ...
    if action == "update":
        ...
        for fid in (args.ids or FEED_IDS):
            if fid not in allowed:
                print(f"  {fid}: not consumed by modlure "
    ...
    if action == "get":
        fid = args.id
        if fid not in allowed:
            print(f"error: {fid} is not consumed by modlure "
    ...
    print("usage: modlure feeds {list|update|get} ...  "
          f"(feeds: {', '.join(FEED_IDS)})", file=__import__('sys').stderr)
    return 2


def add_feeds_subparser(sub) -> None:
    """Register `modlure feeds ...` on an argparse subparsers object."""
    fp = sub.add_parser(
        "feeds",
        ...
        "feeds MODLURE consumes. Supports --offline (air-gap) re-serve.",
    )
